...
import time
import tracemalloc
def analyze_brute_force(start_stop, end_stop):
    """
    Analyze the Brute-Force approach for finding direct routes.
    Measures execution time, memory usage, and intermediate steps.

    Args:
        start_stop (int): The starting stop ID.
        end_stop (int): The ending stop ID.

    Returns:
        dict: A dictionary with execution time, memory usage, and steps taken.
    """
    # Start measuring time and memory
    tracemalloc.start()
    start_time = time.time()
    
    # Run brute-force direct route search
    result = direct_route_brute_force(start_stop, end_stop)
    ...
    # Calculate steps (routes checked)
    total_steps = len(route_to_stops)  # Total routes checked

    # Output analysis
    analysis = {
        'Execution Time (s)': end_time - start_time,
        'Memory Usage (Current, Peak) (MB)': (current / 10**6, peak / 10**6),
...
def analyze_fol_reasoning(start_stop, end_stop):
    """
    Analyze the FOL-based reasoning approach for finding direct routes.
    Measures execution time, memory usage, and intermediate steps.
    ...
    Returns:
        dict: A dictionary with execution time, memory usage, and steps taken.
    """
    # Initialize Datalog environment and add data
    initialize_datalog()
    ...
    # Run FOL-based direct route search
    ...
    # Calculate steps (based on query structure in Datalog)
    # PyDatalog doesn’t expose internal steps, but setup costs can be estimated.
    setup_steps = len(route_to_stops) * sum(len(stops) for stops in route_to_stops.values())
...
def analyze_forward_chaining(start_stop_id, end_stop_id, stop_id_to_include, max_transfers):
    """
    Analyze the Forward Chaining approach for finding optimal routes.
    Measures execution time, memory usage, and intermediate steps.

    Args:
        start_stop_id (int): The starting stop ID.
        end_stop_id (int): The ending stop ID.
        stop_id_to_include (int): The intermediate stop ID.
    ...
    # Run forward chaining search
    result = forward_chaining(start_stop_id, end_stop_id, stop_id_to_include, max_transfers)
    ...
    # Total steps (for analysis, assume one setup step per `DirectRoute` and `OptimalRoute` predicate usage)
    setup_steps = len(route_to_stops) * sum(len(stops) for stops in route_to_stops.values())
...
def analyze_backward_chaining(start_stop_id, end_stop_id, stop_id_to_include, max_transfers):
    """
    Analyze the Backward Chaining approach for finding optimal routes.
    Measures execution time, memory usage, and intermediate steps.
    ...
    # Run backward chaining search
    result = backward_chaining(start_stop_id, end_stop_id, stop_id_to_include, max_transfers)
    ...
    # Total steps (assumed same step count as forward chaining for setup analysis)
    setup_steps = len(route_to_stops) * sum(len(stops) for stops in route_to_stops.values())
...
if __name__ == "__main__":
    ...
    brute_force_analysis = analyze_brute_force(2573, 1177)
    print("Brute-Force Analysis:", brute_force_analysis)

    fol_reasoning_analysis = analyze_fol_reasoning(2573, 1177)
    print("FOL-Based Reasoning Analysis:", fol_reasoning_analysis)

    forward_chaining_analysis = analyze_forward_chaining(22540, 2573, 4686, 1)
    print("Forward Chaining Analysis:", forward_chaining_analysis)

    backward_chaining_analysis = analyze_backward_chaining(22540, 2573, 4686, 1)
    print("Backward Chaining Analysis:", backward_chaining_analysis)
